...
const UIOverlay: React.FC<UIOverlayProps> = ({
  ...
  return (
    <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-2 md:p-4 font-sans z-10">
      ...
      {/* Building Inspector Panel (Center Bottom) */}
      {selectedTile && (
        <div className="absolute bottom-32 left-1/2 transform -translate-x-1/2 w-64 md:w-80 bg-slate-900/95 text-white rounded-xl border-2 border-cyan-500/50 shadow-2xl backdrop-blur-xl pointer-events-auto p-4 z-50 animate-fade-in">
             <div className="flex justify-between items-start mb-2">
                 <div>
                    <h3 className="text-sm font-bold text-cyan-400 uppercase tracking-wider">{BUILDINGS[selectedTile.buildingType].name}</h3>
                    <div className="text-xs text-slate-400 font-mono">Level {selectedTile.level || 1}</div>
                 </div>
                 <button onClick={onCloseSelection} className="text-slate-500 hover:text-white transition-colors">✕</button>
             </div>
             ...
             <div className="space-y-2 mb-4">
                 <div className="flex justify-between text-xs border-b border-slate-700 pb-1">
                     ...
                     <span className="text-green-400 font-mono">
                         +${(BUILDINGS[selectedTile.buildingType].incomeGen * (selectedTile.level || 1)).toFixed(0)}
                         {(selectedTile.level || 1) < 5 && <span className="text-slate-500 text-[10px] ml-1">→ +${(BUILDINGS[selectedTile.buildingType].incomeGen * ((selectedTile.level || 1) + 1)).toFixed(0)}</span>}
                     </span>
                 </div>
                 <div className="flex justify-between text-xs border-b border-slate-700 pb-1">
                     <span className="text-slate-400">Population</span>
                     <span className="text-blue-400 font-mono">
                         +{(BUILDINGS[selectedTile.buildingType].popGen * (selectedTile.level || 1)).toFixed(0)}
                         {(selectedTile.level || 1) < 5 && <span className="text-slate-500 text-[10px] ml-1">→ +{(BUILDINGS[selectedTile.buildingType].popGen * ((selectedTile.level || 1) + 1)).toFixed(0)}</span>}
                     </span>
                 </div>
             </div>

             {(selectedTile.level || 1) < 5 ? (
                <button 
                    onClick={onUpgrade}
                    className="w-full py-2 bg-gradient-to-r from-cyan-700 to-blue-700 hover:from-cyan-600 hover:to-blue-600 rounded text-xs font-bold uppercase tracking-wider shadow-lg border border-cyan-500/30 flex items-center justify-center gap-2"
                >
                    <span>Upgrade</span>
                    <span className="bg-black/30 px-1.5 py-0.5 rounded text-cyan-200 font-mono">${Math.floor(BUILDINGS[selectedTile.buildingType].cost * Math.pow(1.5, selectedTile.level || 1))}</span>
                </button>
             ) : (
                 <div className="text-center text-xs text-yellow-500 font-bold uppercase tracking-widest py-2 border border-yellow-500/30 rounded bg-yellow-900/10">
                     Max Level
                 </div>
             )}
        </div>
      ...
      <div className="flex flex-col-reverse md:flex-row md:justify-between md:items-end pointer-events-auto mt-auto gap-2 w-full max-w-full">
        ...
        {/* Toolbar - Reversed on Mobile so it appears below News */}
      ...
      <div className="absolute bottom-1 right-2 md:right-4 text-[8px] md:text-[9px] text-white/30 font-mono text-right pointer-events-auto hover:text-white/60 transition-colors">
        <a href="https://www.linkedin.com/in/harishik-dev-singh-jamwal-b31b60288" target="_blank" rel="noreferrer">Created by @Harishik</a>
      </div>
    </div>
  );
};
